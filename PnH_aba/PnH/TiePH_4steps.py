...
class RunModel:
    ...
    def interactionSet(self):  # don't edit
        if self.dual:
            for i in range(1,self.tot_rot+1):
                ...
                mdb.models['Model-'+str(self.iteration)].interactions['ContactR'+str(i)].deactivate('Inserting')

            # inserting
            mdb.models['Model-'+str(self.iteration)].ContactProperty('FrictionI')
            mdb.models['Model-'+str(self.iteration)].interactionProperties['FrictionI'].TangentialBehavior(
                dependencies=0, directionality=ISOTROPIC, elasticSlipStiffness=None, 
                formulation=PENALTY, fraction=0.005, maximumElasticSlip=FRACTION, 
                pressureDependency=OFF, shearStressLimit=None, slipRateDependency=OFF, 
                table=((0.3, ), ), temperatureDependency=OFF)
            mdb.models['Model-'+str(self.iteration)].ContactExp(createStepName='Inserting', name='ContactI')
            mdb.models['Model-'+str(self.iteration)].interactions['ContactI'].includedPairs.setValuesInStep(
                stepName='Inserting', useAllstar=ON)
            mdb.models['Model-'+str(self.iteration)].interactions['ContactI'].contactPropertyAssignments.appendInStep(
                assignments=((GLOBAL, SELF, 'FrictionI'), ), stepName='Inserting')
    ...
    def bondary_conditionSet(self):
        if self.dual:
            ...
            mdb.models['Model-'+str(self.iteration)].EncastreBC(createStepName='Inserting', localCsys=None, 
                name='HOLE_fixedI', region=Region(referencePoints=(
                mdb.models['Model-'+str(self.iteration)].rootAssembly.instances['HOLE-1'].referencePoints[2], 
                )))     # fixed hole for inserting
            ...
            mdb.models['Model-'+str(self.iteration)].boundaryConditions['PEG_maniR1'].deactivate('Inserting')
            ...
            mdb.models['Model-'+str(self.iteration)].boundaryConditions['PEG_maniR2'].deactivate('Inserting')
            ...
            mdb.models['Model-'+str(self.iteration)].boundaryConditions['PEG_maniR3'].deactivate('Inserting')
            # R4
            mdb.models['Model-'+str(self.iteration)].DisplacementBC(amplitude='Amp-1', createStepName=
            ...
            mdb.models['Model-'+str(self.iteration)].boundaryConditions['PEG_maniR4'].deactivate('Inserting')
            # R5
            mdb.models['Model-'+str(self.iteration)].DisplacementBC(amplitude='Amp-1', createStepName=
            ...
            mdb.models['Model-'+str(self.iteration)].boundaryConditions['PEG_maniR5'].deactivate('Inserting')
            ...
            mdb.models['Model-'+str(self.iteration)].DisplacementBC(amplitude='Amp-2', createStepName=
                ...
                None, name='PEG_maniI', region=Region(referencePoints=(
                mdb.models['Model-'+str(self.iteration)].rootAssembly.instances['CLAMP-1'].referencePoints[2], 
                )), u1=0.0, u2= -P1Height/2, u3=0, ur1=0, ur2=0, ur3=0)
    ...
    def jobSet(self):               # don't edit
        mdb.Job(atTime=None, contactPrint=OFF, description='', echoPrint=OFF, 
            ...
            PERCENTAGE, model=('Model-'+str(self.iteration)), modelPrint=OFF, name=('Job'+str(self.iteration)), 
        ...
        mdb.jobs['Job'+str(self.iteration)].setValues(explicitPrecision=DOUBLE_PLUS_PACK)
    ...
    def jobSubmitment(self):
        ...
        mdb.jobs['Job'+str(self.iteration)].submit(consistencyChecking=OFF)
        mdb.jobs['Job'+str(self.iteration)].waitForCompletion()
        return None

# The objective function of genetic algorithm for the rotation optimization
# def ObjFunc(rot):
#     # rot[0] = overrot
#     # rot[1] = rotbac
#     PnH_op = RunModel(i*100000+count,1,rot[0],rot[1])
#     SimCount()
